...
namespace YARG.Net.LobbyServer
{
    /// <summary>
    /// Client for interacting with lobby server lobby code endpoints.
    ...
    public sealed class LobbyCodeClient : IDisposable
    {
        ...
        /// <summary>
        /// Registers a lobby with the lobby server.
        /// This must be called before generating a code for the lobby.
        /// </summary>
        /// <param name="lobbyServerUrl">The base URL of the lobby server.</param>
        ...
        {
            if (string.IsNullOrWhiteSpace(lobbyServerUrl))
                return false;
            ...
            try
            {
                var url = lobbyServerUrl.TrimEnd('/') + "/api/lobbies";
                var request = new
                {
                    LobbyId = lobbyId,
        ...
        {
            if (string.IsNullOrWhiteSpace(lobbyServerUrl))
                throw new ArgumentException("Lobby server URL is required", nameof(lobbyServerUrl));
            ...
            try
            {
                var url = BuildCodeUrl(lobbyServerUrl);
        ...
        /// <summary>
        /// Registers an existing lobby code with a lobby server.
        /// This is used to disseminate a code generated by another lobby server
        /// so players using different lobby servers can still find each other.
        /// </summary>
        /// <param name="lobbyServerUrl">The base URL of the lobby server.</param>
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                var url = BuildCodeUrl(lobbyServerUrl) + "/register";
                var request = new 
                { 
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                var url = BuildCodeUrl(lobbyServerUrl, code);
                ...
                if (response.IsSuccessStatusCode)
                {
                    ...
                    if (entry != null)
                    {
                        // Pass the lobby server URL for NAT punch coordination
                        return LobbyLookupResult.Success(entry, lobbyServerUrl);
                    }
                }
    ...
    public sealed class LobbyLookupResult
    {
        ...
        /// <summary>
        /// The lobby server URL that was used for the successful lookup.
        /// Used for NAT punch coordination.
        /// </summary>
        public string? LobbyServerUrl { get; }

        private LobbyLookupResult(bool isSuccess, LobbyDirectoryEntry? lobby, string? error, string? lobbyServerUrl = null)
        {
            IsSuccess = isSuccess;
            Lobby = lobby;
            Error = error;
            LobbyServerUrl = lobbyServerUrl;
        }

        public static LobbyLookupResult Success(LobbyDirectoryEntry lobby, string? lobbyServerUrl = null) =>
            new(true, lobby, null, lobbyServerUrl);
